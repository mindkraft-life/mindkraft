        ...
        async function loadUserData(uid) {
            ...
            if (userDoc && userDoc.exists()) {
                ...
                // Only write if we actually got a network response (not just cache miss)
                if (userDoc !== null) {
        ...
        function processStreakSystem(activity, today) {
            ...
            // activities the current open window can last 7–30 days. If the user
            // completed in THIS week/month and processStreakSystem fires on a
            // subsequent day within the same window, the walk's cursor starts at
            // the PREVIOUS closed window (already below streakStartFloor) and
            // immediately hits the floor with streak=0 — wiping a valid streak
            // and clearing streakStartWindow, which then causes shields to be
            // burned across the gap on the NEXT open-window day.
            //
            // Guard: if the walk returned 0 but the stored streak is positive and
            // streakStartWindow sits inside the CURRENT open window, the walk had
            // nothing to verify yet — completeActivity already owns this window.
            // Leave streak and streakStartWindow untouched.
            if (finalStreak === 0 && (activity.streak || 0) > 0 && activity.streakStartWindow) {
                ...
                if (swWindow && todayWindow && swWindow.getTime() === todayWindow.getTime()) {
                    // Streak started in the current open window — nothing to adjudicate yet.
                    return true;
                }
            }
        ...
        function migrateStreakStartWindows() {
            ...
            // ── V1 migration (already ran for most users, kept for new installs) ──
            (window.userData.dimensions || []).forEach(dim =>
                (dim.paths || []).forEach(path =>
                    (path.activities || []).forEach(act => {
                        ...
                        changed = true;
                        // V2 will handle the real healing; V1 just stamps the guard.
                    })));

            // ── V2 migration (v70) — fixes wrong windowsBack formula in V1 ──────
            // V1 used (streak + shieldsConsumed) windows back as the floor. For an
            // activity with streak=1 and shieldsConsumed=3 (all gap-bug), that put
            // the floor 4 windows back. processStreakSystem then walked the 3 empty
            // gap windows and re-burned all 3 shields immediately after V1 reset them.
            //
            // V2 fix: floor = max(createdAt window, (streak + MAX_SHIELDS) windows back)
            // The createdAt clamp prevents the floor from predating the activity —
            // which is always the real lower bound for weekly/biweekly/monthly activities
            // that were new when the bug hit.
            //
            // We also clear lastProcessedDate so processStreakSystem re-runs fresh
            // with the corrected floor on this same login.
            const MAX_SHIELDS_MIGRATION = 3;
            (window.userData.dimensions || []).forEach(dim =>
                (dim.paths || []).forEach(path =>
                    (path.activities || []).forEach(act => {
                        if (act.streakShieldsMigratedV4) return;
                        act.streakShieldsMigratedV4 = true;
                        changed = true;
                        ...
                        // Correct floor: step back (streak + 3) windows max,
                        // then clamp to createdAt window (take the more recent).
                        const windowsBack = (act.streak || 0) + MAX_SHIELDS_MIGRATION;
                        let stepCursor = getCycleWindowStart(act, new Date());
                        for (let i = 0; i < windowsBack && stepCursor; i++) {
                            stepCursor = getCycleWindowStart(act, new Date(stepCursor.getTime() - 1));
                        }

                        // createdAt clamp — activity cannot predate its own creation
                        let createdAtFloor = null;
                        if (act.createdAt) {
                            const ca = new Date(act.createdAt);
                            ca.setHours(0, 0, 0, 0);
                            createdAtFloor = getCycleWindowStart(act, ca) || ca;
                        }

                        // Use whichever floor is MORE RECENT (tighter bound).
                        // Also include any existing streakStartWindow — if completeActivity
                        // correctly stamped it post-v69 (e.g. "2025-04-20"), that stamp is
                        // more accurate than our approximation and must not be overwritten
                        // with a looser (earlier) value.
                        const floorCandidates = [stepCursor, createdAtFloor].filter(Boolean);
                        if (act.streakStartWindow) {
                            try {
                                const existingSW = getCycleWindowStart(act, new Date(act.streakStartWindow + 'T00:00:00'));
                                if (existingSW) floorCandidates.push(existingSW);
                            } catch(e) {}
                        }
                        const correctFloor = floorCandidates.length > 0
                            ? floorCandidates.reduce((a, b) => a > b ? a : b)
                            : stepCursor;

                        if (correctFloor) act.streakStartWindow = toLocalDateStr(correctFloor);

                        // Recover streakStartWindow if it was cleared mid-week by the
                        // open-window wipe bug. Use lastCompleted to find which window
                        // the streak actually started in — for streak=1 activities this
                        // is always the window containing lastCompleted.
                        if (!act.streakStartWindow && act.lastCompleted && (act.streak || 0) > 0) {
                            const lc = new Date(act.lastCompleted);
                            const lcWindow = getCycleWindowStart(act, lc);
                            if (lcWindow) act.streakStartWindow = toLocalDateStr(lcWindow);
                        }

                        // Reset shields — let processStreakSystem recount real misses
                        act.shieldsConsumed = 0;

                        // Force processStreakSystem to re-run this login with the
                        // corrected floor (clears the once-per-day idempotency guard).
                        act.lastProcessedDate = null;
                    })));
