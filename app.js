        ...
        function renderActivitiesList() {
            ...
            // Update sub-tab pill with activity count
            const _pill = document.getElementById('subTabMyActivities');
            if (_pill) {
                const { total: _actT, limit: _actL } = getActivityCounts();
                _pill.innerHTML = 'My Activities <span class="sub-tab-count">' + _actT + '/' + _actL + '</span>';
            }
            ...
            if (sort === 'smart') {
                ...
                // Within To Do: non-occasional first (by XP desc), then occasional (by XP desc)
                var toDoRegular = toDo.filter(function(a) { return a.frequency !== 'occasional'; })
                    .sort(function(a, b) { return (b.baseXP || 0) - (a.baseXP || 0); });
                var toDoOcc = toDo.filter(function(a) { return a.frequency === 'occasional'; })
                    .sort(function(a, b) { return (b.baseXP || 0) - (a.baseXP || 0); });
                var toDoSorted = toDoRegular.concat(toDoOcc);
                ...
                var smartGroups = [
                    { key: 'smart_todo',   label: '⚡ To Do',       activities: toDoSorted },
                    { key: 'smart_done',   label: '✓ Done Today',   activities: doneTd },
        ...
        function renderActivityCards(activities) {
            ...
            return activities.map(activity => {
                ...
                // Undo button (lives inside expanded area)
                ...
                // ── Collapsed row: Name | XP | streak | check ──
                // ── Expanded area: path, shields, challenge, penalty, undo ──
                const detailBadges = [];
                ...
                <div class="activity-item ${itemClass}" onclick="${clickHandler}">
                    <div class="activity-info-container">
                        ...
                        <div class="activity-expand-body ${isExpanded ? 'expanded' : ''}">
                            ...
                            ${showUndo ? '<div class="activity-undo-row">' + undoBtn + '</div>' : ''}
                        </div>
                    </div>
