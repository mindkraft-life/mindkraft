        ...
        function renderProfileSpiderChart() {
            ...
            ctx.beginPath();
            cats.forEach((c, i) => {
                const r = R * (MIN_FRAC + (1 - MIN_FRAC) * (catXP[c.id] / maxXP));
                const x = cx + r * Math.cos(angle(i));
                const y = cy + r * Math.sin(angle(i));
                i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
            });
            ...
            cats.forEach((c, i) => {
                const xp = catXP[c.id];
                if (xp <= 0) return;
                const r = R * (MIN_FRAC + (1 - MIN_FRAC) * (xp / maxXP));
                const x = cx + r * Math.cos(angle(i));
                const y = cy + r * Math.sin(angle(i));
        ...
        window.openFriendProfileCard = function(uid) {
            ...
            const polygon   = cats.map((c, i) => {
                const FRIEND_MIN = 0.05;
                const pct   = FRIEND_MIN + (1 - FRIEND_MIN) * Math.min(1, (catXP[c.id] || 0) / maxXP);
