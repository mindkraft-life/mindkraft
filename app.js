        ...
        function renderPlanner() {
            ...
            // Add recurring items (unless skipped for this day)
            (window.userData.planner.recurring || []).forEach(function(rec) {
                ...
                merged.push({
                    ...
                    title: rec.title || '',
                    isRecurring: true,
                    completed: false // recurring items check activity state
                });
            });
            ...
            // For activity-linked items, check real completion state
            ...
            merged.forEach(function(item) {
                var timeLabel = item.time ? formatTime12(item.time) : '';
                var showTimeHeader = item.time && item.time !== lastTimeLabel;
                lastTimeLabel = item.time || lastTimeLabel;

                if (showTimeHeader) {
                    html += '<div class="planner-time-label">' + timeLabel + '</div>';
                }
                ...
                html += '<div class="' + cardClass + '"'
                    ...
                    + '<button class="planner-del-btn" onclick="event.stopPropagation();deletePlannerItem(\'' + item.id + '\',' + item.isRecurring + ')" title="Remove">✕</button>'
                    + '</div>'
                    + '</div>';
            });
        ...
        // ── Delete planner item ──
        window.deletePlannerItem = async function(itemId, isRecurring) {
            ensurePlannerData();
            if (isRecurring) {
                // Ask: remove just for today or permanently?
                var choice = confirm('This repeats daily.\n\nOK = Delete permanently\nCancel = Remove just for today');
                if (choice) {
                    // Delete from recurring array
