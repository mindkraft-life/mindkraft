        ...
        onAuthStateChanged(auth, async (user) => {
            ...
            try {
                if (user) {
                    ...
                    // Ensure public profile exists for friends system (non-blocking)
                    syncPublicProfile().catch(e => console.warn('Initial profile sync:', e));
        ...
        async function syncPublicProfile() {
            if (Date.now() - _lastPublicProfileSync < 30000) return;
        ...
        function handleAddFriendDeepLink() {
            ...
            if ((window.userData.friendUids || []).includes(friendUid)) return;
            // Switch to friends tab and show the add confirmation card
            setTimeout(async () => {
                try {
                    ...
                    showAddFriendConfirmCard(friendUid, data);
                } catch (e) { console.error('Deep link error:', e); }
            }, 600);
        }

        function showAddFriendConfirmCard(friendUid, profile) {
            const existing = document.getElementById('addFriendConfirmCard');
            if (existing) existing.remove();
            const card = document.createElement('div');
            card.id = 'addFriendConfirmCard';
            card.className = 'modal-overlay';
            card.style.display = 'flex';
            card.innerHTML = `
                <div class="fl-confirm-card">
                    <div class="fl-confirm-avatar-wrap">
                        <img src="${escapeHtml(profile.photoURL || '')}" onerror="this.style.display='none'" class="fl-confirm-avatar" alt="">
                        <div class="fl-confirm-avatar-fallback">${(profile.displayName || '?')[0]}</div>
                    </div>
                    <div class="fl-confirm-name">${escapeHtml(profile.displayName || 'Unknown')}</div>
                    <div class="fl-confirm-sub">Lv ${profile.level || 1} · ${escapeHtml(profile.title || 'The Initiate')}</div>
                    <div class="fl-confirm-stats">
                        <div class="fl-confirm-stat"><span class="fl-confirm-stat-val">${(profile.totalXP || 0).toLocaleString()}</span><span class="fl-confirm-stat-lbl">Total XP</span></div>
                        <div class="fl-confirm-stat"><span class="fl-confirm-stat-val">${profile.bestStreak || 0}</span><span class="fl-confirm-stat-lbl">Best Streak</span></div>
                        <div class="fl-confirm-stat"><span class="fl-confirm-stat-val">${profile.activeDays || 0}</span><span class="fl-confirm-stat-lbl">Active Days</span></div>
                    </div>
                    <div class="fl-confirm-msg">wants to connect with you on Mindkraft</div>
                    <div class="fl-confirm-actions">
                        <button class="fl-btn-accept" onclick="(async()=>{await addFriendByUid('${friendUid}','${escapeHtml(profile.displayName || '')}');document.getElementById('addFriendConfirmCard')?.remove();})()">Add Friend</button>
                        <button class="fl-btn-skip" onclick="document.getElementById('addFriendConfirmCard')?.remove()">Not Now</button>
                    </div>
                </div>`;
        ...
        function renderFriendsLeaderboard() {
            ...
            el.innerHTML = sorted.map((p, i) => {
                ...
                const clickAttr = p.isMe ? '' : ` onclick="openFriendProfile('${p.uid}')"`;
                return `
                    <div class="fl-lb-card${meClass}"${clickAttr}>
        ...
        window.submitInlineAddFriend = async function() {
            ...
            try {
                ...
                const snap = await getDocs(q);
                if (snap.empty) { status.textContent = 'No user found.'; status.className = 'fl-add-status fl-add-error'; return; }
