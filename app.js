        ...
        function getXPThisWeek() {
            ...
            (window.userData.dimensions || []).forEach(dim => {
                (dim.paths || []).forEach(path => {
                    (path.activities || []).forEach(act => {
                        (act.completionHistory || []).forEach(e => {
                            if (!e.isPenalty && new Date(e.date) >= monday) xp += (e.xp || 0);
                        });
                    });
                });
            });
        ...
        function getYesterdayXPMetrics() {
            ...
            (window.userData.dimensions || []).forEach(dim => {
                (dim.paths || []).forEach(path => {
                    (path.activities || []).forEach(act => {
                        (act.completionHistory || []).forEach(e => {
                            if (!e.isPenalty && new Date(e.date).toDateString() === yStr) {
                                xp += (e.xp || 0);
                            }
                        });
                    });
        ...
        function renderAnalyticsSummary(activities, log) {
            ...
            // Always-global metrics (not affected by scope/period filters)
        ...
        window.openAddFriendModal = function() {
            ...
            if (existing) existing.remove();
            const friendCount = (window.userData.friendUids || []).length;
            ...
            modal.innerHTML = `
                <div class="modal-content" style="max-width:360px;">
                    <div class="modal-header">
                        ...
                        <button class="modal-close" onclick="document.getElementById('addFriendModal').remove()">\u2715</button>
                    </div>
                    <div class="modal-body" style="padding:20px;">
                        ${friendCount >= MAX_FRIENDS ? `
                            <div style="text-align:center;padding:16px;color:var(--color-text-secondary);font-size:14px;">
                                Friend limit reached (${MAX_FRIENDS}).
                            </div>
                        ` : `
                            ...
                            <div style="display:flex;gap:8px;">
                                <input type="text" id="addFriendCodeInput" class="text-input" placeholder="e.g. A3X7K2" maxlength="6"
                                    style="flex:1;font-size:18px;font-family:monospace;text-align:center;letter-spacing:0.15em;text-transform:uppercase;">
                                <button class="btn-primary" onclick="submitAddFriendByCode()">Add</button>
                            </div>
                            <div id="addFriendStatus" style="font-size:13px;min-height:20px;margin-top:10px;text-align:center;"></div>
                        `}
                    </div>
                </div>`;
            document.body.appendChild(modal);
            modal.addEventListener('click', e => { if (e.target === modal) modal.remove(); });
            document.getElementById('addFriendCodeInput')?.focus();
        };

        window.submitAddFriendByCode = async function() {
            const raw = (document.getElementById('addFriendCodeInput')?.value || '').trim().toUpperCase();
            ...
            try {
                ...
                setTimeout(() => { document.getElementById('addFriendModal')?.remove(); }, 1000);
            } catch (e) {
        ...
        async function renderFriendsTab() {
            ...
            if (uids.length === 0) {
                container.innerHTML = `
                    <div style="padding:20px;">
                        ...
                        <button class="btn-primary" onclick="openAddFriendModal()" style="width:100%;">+ Add Friend</button>
                    </div>`;
            ...
            container.innerHTML = `
                <div style="padding:16px;">
                    ...
                    <!-- Add Friend button -->
                    <button class="btn-primary" onclick="openAddFriendModal()" style="width:100%;margin-bottom:16px;">+ Add Friend</button>
