        ...
        // ── State ──
        window._plannerDate = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
        ...
        window.plannerDateNav = function(delta) {
            ...
            window._plannerDate = d.toISOString().slice(0, 10);
            renderPlanner();
        };

        window.setPlannerDate = function(val) {
            if (!val) {
                window._plannerDate = new Date().toISOString().slice(0, 10);
            } else {
        ...
        function positionNowMarker() {
            ...
            var line = document.getElementById('plannerNowLine');
            if (!marker || !line) return;

            var now = new Date();
            var nowTime = (now.getHours() < 10 ? '0' : '') + now.getHours() + ':' + (now.getMinutes() < 10 ? '0' : '') + now.getMinutes();

            // Find the time rows and position marker relative to them
            var wrap = document.querySelector('.planner-timeline-wrap');
            if (!wrap) return;

            var timeRows = wrap.querySelectorAll('.planner-time-row');
            if (timeRows.length === 0) { marker.style.display = 'none'; line.style.display = 'none'; return; }

            // Find position between time rows
            var prevBottom = 0;
            var placed = false;

            for (var i = 0; i < timeRows.length; i++) {
                var rowTime = timeRows[i].getAttribute('data-time') || '99:99';
                if (nowTime < rowTime) {
                    // Place before this row
                    var rowTop = timeRows[i].offsetTop;
                    var yPos = (prevBottom + rowTop) / 2;
                ...
                prevBottom = timeRows[i].offsetTop + timeRows[i].offsetHeight;
            }

            if (!placed) {
                // After all time rows
                marker.style.top = (prevBottom + 12) + 'px';
                marker.style.display = '';
                line.style.top = (prevBottom + 12) + 'px';
                line.style.display = '';
            }
        }
