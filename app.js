        ...
        window.completeChallenge = async function(index) {
            ...
            await saveUserData();
            updateDashboard();
            // Push completed status to group challenge so other members see 100% / ✓ All done
        ...
        // ── Take Again / Try Again: reset a completed or failed challenge ──
        window.takeAgainChallenge = async function(index) {
            const challenge = window.userData.challenges[index];
            if (!challenge) return;
            const wasCompleted = challenge.status === 'completed';
            const label = wasCompleted ? 'take this challenge again' : 'try this challenge again';
            if (!confirm(`Ready to ${label}? Progress will be reset and you can choose new dates.`)) return;

            // Reset progress fields
            challenge.status = 'active';
            challenge.currentCount = 0;
            challenge.metricCurrent = 0;
            challenge.activityProgressCollapsed = true;
            if (challenge.activityProgress) {
                Object.keys(challenge.activityProgress).forEach(k => { challenge.activityProgress[k] = 0; });
            }
            // Clear dates so the edit modal prompts for new ones
            challenge.startDate = '';
            challenge.endDate = '';

            await saveUserData();
            // Open the edit modal so user can set new dates
            openChallengeModal(index);
        };
        ...
        // Render Challenges
        // Session-level collapse state for completed/failed sections (collapsed by default)
        if (!window._chSectionCollapsed) window._chSectionCollapsed = { completed: true, failed: true };

        window.toggleChallengeSection = function(section) {
            window._chSectionCollapsed[section] = !window._chSectionCollapsed[section];
            renderChallenges();
        };
        ...
        function renderChallenges() {
            ...
            if (completedChallenges.length > 0) {
                const isCollapsed = window._chSectionCollapsed.completed;
                html += `
                <div class="ch-section-header" onclick="toggleChallengeSection('completed')" style="display:flex;align-items:center;justify-content:space-between;cursor:pointer;margin: 24px 0 12px 0;user-select:none;-webkit-user-select:none;">
                    <h3 style="font-size: 18px;margin:0;">Completed <span style="font-size:13px;font-weight:500;color:var(--color-text-secondary);">(${completedChallenges.length})</span></h3>
                    <span style="font-size:13px;color:var(--color-text-secondary);transition:transform 0.2s;display:inline-block;transform:rotate(${isCollapsed ? '0deg' : '180deg'});">▼</span>
                </div>`;
                if (!isCollapsed) {
            ...
            if (failedChallenges.length > 0) {
                const isCollapsed = window._chSectionCollapsed.failed;
                html += `
                <div class="ch-section-header" onclick="toggleChallengeSection('failed')" style="display:flex;align-items:center;justify-content:space-between;cursor:pointer;margin: 24px 0 12px 0;user-select:none;-webkit-user-select:none;">
                    <h3 style="font-size: 18px;margin:0;">Failed <span style="font-size:13px;font-weight:500;color:var(--color-text-secondary);">(${failedChallenges.length})</span></h3>
                    <span style="font-size:13px;color:var(--color-text-secondary);transition:transform 0.2s;display:inline-block;transform:rotate(${isCollapsed ? '0deg' : '180deg'});">▼</span>
                </div>`;
                if (!isCollapsed) {
        ...
        function renderChallengeCard(challenge, index) {
            ...
            // Name map + baseXP map (used for live bonusXP sync)
            const nameMap = {};
            const baseXPMap = {};
            (window.userData.dimensions || []).forEach(dim =>
                (dim.paths || []).forEach(path =>
                    (path.activities || []).forEach(act => {
                        nameMap[act.id] = act.name;
                        baseXPMap[act.id] = act.baseXP || 1;
                    })));
            ...
            // ── Live bonusXP sync: recompute if activity baseXP has changed ─
            // Only for active challenges with linked activities — completed/failed keep their earned XP frozen.
            if (isActive && hasPerActivity) {
                let recomputedBase = 0;
                challengeActivityIds.forEach(id => {
                    recomputedBase += (baseXPMap[id] || 1) * (challenge.activityTargets[id] || 1);
                });
                const recomputedBonus = Math.max(1, Math.round(recomputedBase * 0.2));
                if (recomputedBonus !== challenge.bonusXP) {
                    challenge.bonusXP = recomputedBonus;
                ...
                <div class="${cardClass}">
                    <div class="ch-header">
                        ...
                        <div class="ch-actions">
                            ...
                            ` : isCompleted ? `
                                <button class="btn-icon" onclick="takeAgainChallenge(${index})" style="border-color:var(--color-accent-blue);color:var(--color-accent-blue);white-space:nowrap;" title="Reset and run again">↺ Take Again</button>
                                <button class="btn-icon delete" onclick="deleteChallenge(${index})" title="Delete">✕</button>
                            ` : isFailed ? `
                                <button class="btn-icon" onclick="takeAgainChallenge(${index})" style="border-color:var(--color-accent-blue);color:var(--color-accent-blue);white-space:nowrap;" title="Try again">↺ Try Again</button>
                                <button class="btn-icon delete" onclick="deleteChallenge(${index})" title="Delete">✕</button>
                            ` : ''}
                        </div>
                    </div>
        ...
        window.renderFriendsTab = async function() {
            ...
            // ── 5. Leaderboard — top 10 including self (respects exclusion list) ─
            const excluded = new Set(window.userData.leaderboardExclusions || []);
            const lbEntries  = entries.filter(e => e.isMe || !excluded.has(e.uid));
            const sorted     = [...lbEntries].sort((a, b) => (b.weeklyXP || 0) - (a.weeklyXP || 0));
        ...
        window.openFriendProfileCard = function(uid) {
            ...
            document.getElementById('friendProfileContent').innerHTML = `
                ...
                ${!isMe ? (() => {
                    const isExcluded = (window.userData.leaderboardExclusions || []).includes(uid);
                    return `
                <div style="display:flex;flex-direction:column;gap:8px;">
                    <button id="lbExcludeBtn" onclick="toggleLeaderboardExclusion('${escapeHtml(uid)}')"
                        style="width:100%;background:none;border:1px solid ${isExcluded ? 'rgba(93,156,236,0.4)' : 'var(--color-border)'};color:${isExcluded ? 'var(--color-accent-blue)' : 'var(--color-text-secondary)'};border-radius:10px;padding:10px;font-size:13px;cursor:pointer;font-family:inherit;">
                        ${isExcluded ? '＋ Add to Leaderboard' : '— Remove from Leaderboard'}
                    </button>
        ...
        // ── Toggle leaderboard exclusion (friend stays in friends list) ───
        window.toggleLeaderboardExclusion = async function(uid) {
            const excl = window.userData.leaderboardExclusions || [];
            const isExcluded = excl.includes(uid);
            window.userData.leaderboardExclusions = isExcluded
                ? excl.filter(id => id !== uid)
                : [...excl, uid];
            await saveUserData();
            // Refresh profile card button text in-place
            const btn = document.getElementById('lbExcludeBtn');
            if (btn) {
                const nowExcluded = (window.userData.leaderboardExclusions || []).includes(uid);
                btn.textContent = nowExcluded ? '＋ Add to Leaderboard' : '— Remove from Leaderboard';
                btn.style.color = nowExcluded ? 'var(--color-accent-blue)' : 'var(--color-text-secondary)';
                btn.style.borderColor = nowExcluded ? 'rgba(93,156,236,0.4)' : 'var(--color-border)';
            }
            // Re-render leaderboard section only (cheap, already have cache)
            renderFriendsTab();
            showToast(isExcluded ? '✓ Added back to leaderboard' : 'Removed from leaderboard', 'blue');
        };
                    ...
                    <div class="gc-progress-section">
                        ...
                        <div class="gc-momentum-row">
                            <div class="gc-momentum-bar-wrap">
                                <div class="gc-momentum-fill" style="width:${momentum}%;background:${momentumColor};"></div>
                                <div class="gc-momentum-wave" style="left:${momentum}%;"></div>
                            </div>
        ...
        // Session-level cache of the last values we wrote to Firestore for each group.
        // Used in gcSyncProgress to compare against OUR last write rather than a potentially
        // stale Firestore cached read — prevents the momentum bar from updating when a
        // non-nominated activity is completed right after a previous sync.
        const _gcSyncBaseline = {}; // { [groupId]: updatedNoms array from last successful write }
        ...
        window.gcSyncProgress = async function() {
            ...
            try {
                ...
                // Compare against OUR last-written baseline first (avoids stale Firestore cache
                // causing false positives when completing non-nominated activities).
                const baseline = _gcSyncBaseline[groupId] || noms;
                const progressMade = updatedNoms.some((n, i) =>
                    (n.challengeCurrent > (baseline[i]?.challengeCurrent || 0)) ||
                    (n.metricCurrent    > (baseline[i]?.metricCurrent    || 0))
                );
                ...
                // Update baseline to what we just wrote so next call compares correctly
                _gcSyncBaseline[groupId] = updatedNoms;
