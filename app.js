        ...
        // ── Friends System ───────────────────────────────────────────────
        // Architecture:
        //   WRITES: syncPublicProfile() runs on every save (throttled 30s)
        //           so weeklyXP stays real-time for anyone viewing their
        //           friends tab.
        //   READS:  All reads deferred to Friends tab open:
        //           - up to 20 friend publicProfiles (cached 60s)
        //           - pending friendRequests query
        //   ADD:    Friend code (UID) only — entered manually or via
        //           shareable link. No email search.
        //   CAP:    Max 20 friends.
        // ══════════════════════════════════════════════════════════════════
        ...
        async function syncPublicProfile() {
            // Throttle to max once per 30s to conserve writes
        ...
        // ── Add Friend (code / link only) ───────────────────────────────

        // Check for ?addFriend=UID in URL on load
        function checkAddFriendFromURL() {
            ...
            setTimeout(() => confirmAddFriendFromLink(friendUid), 800);
        }

        async function confirmAddFriendFromLink(friendUid) {
            try {
                const snap = await getDoc(doc(db, 'publicProfiles', friendUid));
                if (!snap.exists()) { alert('Friend not found — they may not have used Mindkraft yet.'); return; }
                ...
                console.error('Add from link error:', e);
                alert('Could not add friend. Try again later.');
            }
        }
        ...
        window.openAddFriendModal = function() {
                ...
                <div class="modal-content" style="max-width:400px;">
                    ...
                    <div class="modal-body" style="padding:20px;">
                        ${friendCount >= MAX_FRIENDS ? `
                            <div style="padding:16px;background:var(--color-bg-hover);border-radius:10px;text-align:center;margin-bottom:16px;">
                                <div style="font-size:14px;color:var(--color-text-secondary);">You've reached the maximum of ${MAX_FRIENDS} friends.</div>
                            </div>
                        ` : `
                            <label class="field-label">Enter Friend Code</label>
                            <div style="display:flex;gap:8px;margin-bottom:12px;">
                                <input type="text" id="addFriendCode" class="text-input" placeholder="Paste friend code" style="flex:1;font-size:13px;font-family:monospace;">
                                <button class="btn-primary" onclick="submitAddFriendByCode()" style="white-space:nowrap;">Add</button>
                            </div>
                            <div id="addFriendStatus" style="font-size:13px;min-height:20px;margin-bottom:16px;"></div>
                        `}

                        <div style="padding-top:16px;border-top:1px solid var(--color-border);">
                            <label class="field-label">Share Your Link</label>
                            <div style="font-size:12px;color:var(--color-text-secondary);margin-bottom:8px;">Friends can tap this to add you instantly.</div>
                            <div style="display:flex;gap:8px;">
                                <input type="text" class="text-input" value="${shareURL}" readonly style="font-size:11px;flex:1;font-family:monospace;">
                                <button class="filter-pill" onclick="navigator.clipboard.writeText('${shareURL}');this.textContent='Copied!';setTimeout(()=>this.textContent='Copy',1500)">Copy</button>
                            </div>
                            ${navigator.share ? `
                                <button class="btn-primary" onclick="navigator.share({title:'Add me on Mindkraft',url:'${shareURL}'})" style="width:100%;margin-top:10px;">Share Link</button>
                            ` : ''}
                        </div>

                        <div style="padding-top:16px;border-top:1px solid var(--color-border);margin-top:16px;">
                            <label class="field-label">Your Friend Code</label>
                            <div style="display:flex;gap:8px;align-items:center;">
                                <input type="text" class="text-input" value="${window.currentUser?.uid || ''}" readonly style="font-size:11px;flex:1;font-family:monospace;">
                                <button class="filter-pill" onclick="navigator.clipboard.writeText('${window.currentUser?.uid || ''}');this.textContent='Copied!';setTimeout(()=>this.textContent='Copy',1500)">Copy</button>
                            </div>
                        </div>
                    </div>
                </div>`;
            ...
            if (friendCount < MAX_FRIENDS) document.getElementById('addFriendCode')?.focus();
        };
        ...
        window.submitAddFriendByCode = async function() {
            ...
            if (!code) { status.textContent = 'Please paste a friend code.'; status.style.color = 'var(--color-accent-red)'; return; }
            if (code === window.currentUser.uid) { status.textContent = "That's your own code!"; status.style.color = 'var(--color-accent-red)'; return; }
            if ((window.userData.friendUids || []).includes(code)) { status.textContent = 'Already on your friends list!'; status.style.color = 'var(--color-text-secondary)'; return; }
            if ((window.userData.friendUids || []).length >= MAX_FRIENDS) { status.textContent = `Max ${MAX_FRIENDS} friends reached.`; status.style.color = 'var(--color-accent-red)'; return; }

            status.textContent = 'Looking up…';
            status.style.color = 'var(--color-text-secondary)';
            try {
                const profileSnap = await getDoc(doc(db, 'publicProfiles', code));
                ...
                await addFriendByUid(code, data.displayName || 'Unknown');
                status.textContent = `✓ Added ${data.displayName || 'friend'}!`;
                status.style.color = 'var(--color-accent-green)';
                setTimeout(() => { document.getElementById('addFriendModal')?.remove(); }, 1200);
            } catch (e) {
                console.error('Add friend by code error:', e);
        ...
        async function addFriendByUid(friendUid, friendName) {
            ...
            // Send a friend request so they get prompted to add back
            try {
            ...
            } catch (e) { console.warn('Could not send friend request:', e); }

            // Invalidate cache so tab re-renders fresh
            _friendsCacheTime = 0;
            renderFriendsTab();
        }
        ...
        // ── Load Friends Profiles (read-side, deferred to tab open) ─────
        let _friendsCache = [];
        ...
        async function loadFriendsProfiles() {
            ...
            // Cache for 60s so switching tabs doesn't re-fetch
            if (Date.now() - _friendsCacheTime < 60000 && _friendsCache.length) return _friendsCache;

            const profiles = [];
            // Fetch up to MAX_FRIENDS profiles individually
            // (Firestore doesn't support getDoc batching without 'in' queries
            //  and these are document-ID lookups which are fast)
            const toFetch = uids.slice(0, MAX_FRIENDS);
        ...
        // ── Friend Request Check (deferred to tab open) ─────────────────
        let _friendRequestsChecked = false;

        async function checkPendingFriendRequests() {
            if (!window.currentUser || _friendRequestsChecked) return;
            _friendRequestsChecked = true;
            try {
                const q = query(
                ...
                const existingFriends = new Set(window.userData.friendUids || []);
                const pending = [];
                snap.docs.forEach(d => {
                    const data = d.data();
                    if (!existingFriends.has(data.fromUid)) {
                        pending.push({ docId: d.id, ...data });
                    } else {
                        // Already friends — clean up silently
                ...
                console.warn('checkPendingFriendRequests error:', e);
            }
        }
                ...
                <div class="modal-content" style="max-width:400px;">
                    ...
                    <div class="modal-body" style="padding:20px;">
                        <div style="font-size:13px;color:var(--color-text-secondary);margin-bottom:16px;">
                            ${requests.length} ${requests.length === 1 ? 'person has' : 'people have'} added you as a friend!
                        </div>
                        <div id="friendRequestList">
                            ${requests.map(r => `
                                <div class="friend-card" id="friendReq_${r.docId}" style="margin-bottom:8px;">
        ...
        window.acceptFriendRequest = async function(docId, fromUid, fromName) {
            const reqEl = document.getElementById('friendReq_' + docId);
            ...
            if (reqEl) {
                reqEl.innerHTML = `<div style="padding:12px;color:var(--color-accent-green);font-size:13px;">✓ Added ${fromName || 'friend'}!</div>`;
                reqEl.style.opacity = '1';
            }

            _friendsCacheTime = 0;

            setTimeout(() => {
                const remaining = document.querySelectorAll('#friendRequestList .friend-card:not([style*="opacity"])');
                const hasButtons = document.querySelectorAll('#friendRequestList .filter-pill');
                if (hasButtons.length === 0) document.getElementById('friendRequestModal')?.remove();
            }, 1500);
        };
        ...
        window.dismissFriendRequest = async function(docId) {
            ...
            const remaining = document.querySelectorAll('#friendRequestList .friend-card');
            if (remaining.length === 0) document.getElementById('friendRequestModal')?.remove();
        };

        // ── Friends Tab Render ───────────────────────────────────────────
        window._friendsLeaderboardMode = 'weekly'; // 'weekly' or 'xphr'
        ...
        async function renderFriendsTab() {
            ...
            // Check friend requests on first tab open (deferred reads)
            checkPendingFriendRequests().catch(e => {});

            // Also check for addFriend URL param
            checkAddFriendFromURL();
            ...
            if (uids.length === 0) {
                container.innerHTML = `
                    <div style="padding:16px;">
                        <button class="btn-primary" onclick="openAddFriendModal()" style="width:100%;margin-bottom:20px;">+ Add Friend</button>
                        <div class="empty-placeholder">
                            <div style="font-size:36px;margin-bottom:12px;">👥</div>
                            <div style="font-size:15px;font-weight:600;color:var(--color-text-primary);margin-bottom:6px;">No friends yet</div>
                            <div style="font-size:13px;color:var(--color-text-secondary);line-height:1.5;">Add friends by code or share your link to compare progress and compete on the leaderboard.</div>
                        </div>
                    </div>`;
                return;
            }
            ...
            container.innerHTML = `
                <div style="padding:16px;">
                    ...
                    <div style="text-align:center;padding:30px 0;color:var(--color-text-secondary);">Loading friends…</div>
                </div>`;
            ...
            container.innerHTML = `
                <div style="padding:16px;">
                    ...
                    <div class="analytics-card" style="margin-bottom:16px;">
                        <div class="analytics-card-header" style="flex-wrap:wrap;gap:8px;">
                            ...
                            <div class="filter-pills" style="gap:4px;">
                                ...
                                <button class="filter-pill ${window._friendsLeaderboardMode === 'xphr' ? 'active' : ''}" onclick="setFriendsLeaderboardMode('xphr',this)">XP/Hr Yesterday</button>
                            </div>
                        </div>
                    ...
                    <!-- Friends List -->
                    <div style="font-size:13px;color:var(--color-text-secondary);margin-bottom:8px;">${friends.length} friend${friends.length !== 1 ? 's' : ''} · max ${MAX_FRIENDS}</div>
                    <div id="friendsListCards">
                        ${friends.map(f => `
                            <div class="friend-card">
                                ...
                                <button class="friend-remove-btn" onclick="removeFriend('${f.uid}')" title="Remove friend">✕</button>
                            </div>
                        `).join('')}
                    </div>
                </div>`;
        ...
        function renderFriendsLeaderboard() {
            ...
            if (data.length === 0) { el.innerHTML = '<div class="empty-state" style="padding:16px 0;"><p>No data</p></div>'; return; }

            const isWeekly = window._friendsLeaderboardMode === 'weekly';
            const sorted = [...data].sort((a, b) => {
                const aVal = isWeekly ? (a.weeklyXP || 0) : (a.xpPerHr || 0);
                const bVal = isWeekly ? (b.weeklyXP || 0) : (b.xpPerHr || 0);
                return bVal - aVal;
            });
            ...
            el.innerHTML = sorted.map((p, i) => {
                ...
                return `<div class="rank-row" style="${p.isMe ? 'background:var(--color-bg-hover);border-radius:8px;' : ''}">
                    <span class="rank-num">${i < 3 ? medals[i] : '#' + (i + 1)}</span>
                    <span class="rank-label" style="${nameStyle}" title="${escapeHtml(p.displayName || '')}">${escapeHtml(p.displayName || 'Unknown')}</span>
                    <div class="rank-bar-track">
        ...
        // ── Handle ?addFriend=UID deep link ─────────────────────────────
        function handleAddFriendDeepLink() {
            ...
            (async () => {
                try {
                    ...
                    if (!profileSnap.exists()) { alert('Friend not found.'); return; }
                    const data = profileSnap.data();
                    if (confirm('Add ' + (data.displayName || 'this user') + ' as a friend?')) {
                        await addFriendByUid(friendUid, data.displayName || 'Unknown');
                        alert('\u2713 Added ' + (data.displayName || 'friend') + '!');
                    }
                } catch (e) { console.error('Deep link add friend error:', e); }
            })();
        }
