        ...
        function renderActivitiesList() {
            ...
            if (sort === 'smart') {
                ...
                allActivities.forEach(function(a) {
                    var completed = isCompletedToday(a);
                    var canDo = canCompleteActivity(a);
                    var isMulti = a.allowMultiplePerDay && a.frequency !== 'occasional';
                    var notScheduled = a.frequency === 'custom' && a.customSubtype === 'days' && !isScheduledDay(a);
                    var doneAnythingToday = countCompletionsToday(a) > 0;
        ...
        function renderActivityGridCards(activities) {
            ...
            activities.forEach(function(activity) {
                ...
                var currentStreak = isOccasional ? 0 : calculateStreak(activity);
                ...
                var todayCompletionCount = countCompletionsToday(activity);

                // Shield info
                var shieldsUsed = isOccasional ? 0 : getShieldsUsedDisplay(activity);
                var shieldsLeft = Math.max(0, MAX_SHIELDS - shieldsUsed);

                // Custom counter
                var notScheduledToday = activity.frequency === 'custom' && activity.customSubtype === 'days' && !isScheduledDay(activity);
                var counterText = '';
                if (activity.frequency === 'custom') {
                    var done = cycleCompletionsNow(activity);
        ...
        function renderPlanner() {
            ...
            html += '<div class="planner-timeline-line"></div>';

            // Current time marker (only for today)
            if (isToday) {
                var now = new Date();
                var nowMinutes = now.getHours() * 60 + now.getMinutes();
                // We'll position it after rendering using JS
                html += '<div class="planner-now-marker" id="plannerNowMarker" style="display:none;"></div>';
                html += '<div class="planner-now-line" id="plannerNowLine" style="display:none;"></div>';
            }
            ...
            timedItems.forEach(function(item) {
                var showTimeHeader = item.time !== lastTimeLabel;
                lastTimeLabel = item.time;

                if (showTimeHeader) {
                    // Time dot on the timeline
                    var dotClass = 'planner-time-dot';
                    if (item.completed) dotClass += ' dot-done';
                    html += '<div class="planner-time-row" data-time="' + item.time + '">';
                    html += '<div class="' + dotClass + '"></div>';
                    html += '<div class="planner-time-label">' + formatTime12(item.time) + '</div>';
                    html += '</div>';
                }
            ...
            if (untimedItems.length > 0) {
                html += '<div class="planner-time-row">';
                html += '<div class="planner-time-dot dot-note"></div>';
        ...
        // Position the current-time marker along the timeline
        function positionNowMarker() {
            var marker = document.getElementById('plannerNowMarker');
            var nowLine = document.getElementById('plannerNowLine');
            if (!marker || !nowLine) return;

            var wrap = document.querySelector('.planner-timeline-wrap');
            if (!wrap) return;

            var now = new Date();
            var nowTime = (now.getHours() < 10 ? '0' : '') + now.getHours() + ':' + (now.getMinutes() < 10 ? '0' : '') + now.getMinutes();

            var timeRows = wrap.querySelectorAll('.planner-time-row[data-time]');
            ...
            // Collect time row positions
            var positions = [];
            timeRows.forEach(function(row) {
                var t = row.getAttribute('data-time') || '';
                var top = row.offsetTop + row.offsetHeight / 2;
                positions.push({ time: t, y: top });
            });

            var yPos;
            if (nowTime <= positions[0].time) {
                // Before first item
                yPos = Math.max(0, positions[0].y - 20);
            } else if (nowTime >= positions[positions.length - 1].time) {
                // After last item
                yPos = positions[positions.length - 1].y + 30;
            } else {
                // Between items — interpolate
                for (var i = 0; i < positions.length - 1; i++) {
                    if (nowTime >= positions[i].time && nowTime < positions[i + 1].time) {
                        var t1 = timeToMinutes(positions[i].time);
                        var t2 = timeToMinutes(positions[i + 1].time);
                        var tNow = timeToMinutes(nowTime);
                        var frac = (t2 - t1) > 0 ? (tNow - t1) / (t2 - t1) : 0;
                        yPos = positions[i].y + frac * (positions[i + 1].y - positions[i].y);
                        break;
                    }
                }
            }
        ...
        // Auto-update now marker every 60 seconds
        setInterval(function() {
            var todayStr = localDateStr();
            if (window._plannerDate === todayStr) {
                var marker = document.getElementById('plannerNowMarker');
                if (marker) positionNowMarker();
            }
        }, 60000);
