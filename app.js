        ...
        // ISO week label e.g. "2026-W14" - used to detect stale weeklyXP in public profiles
        ...
        window.renderFriendsTab = async function() {
            ...
            // Build my own entry from live data
            const catXP = getProfileCategoryXP();
            const myEntry = {
                uid:            myUID,
                displayName:    (window.userData.profile && window.userData.profile.username) || window.currentUser.displayName || 'You',
                ...
                bestStreak:     (() => {
                    const a = [];
                    (window.userData.dimensions||[]).forEach(d=>(d.paths||[]).forEach(p=>(p.activities||[]).forEach(x=>a.push(x))));
                    return a.reduce((m,x)=>Math.max(m,x.bestStreak||x.streak||0),0);
                })(),
                activeDays: (() => {
                    const s = new Set();
                    (window.userData.dimensions||[]).forEach(d=>(d.paths||[]).forEach(p=>(p.activities||[]).forEach(a=>{
                        (a.completionHistory||[]).forEach(e=>{if(!e.isPenalty&&e.date)s.add(e.date.slice(0,10));});
                    })));
                    return s.size;
                })(),
                isMe: true
            ...
            // Fetch friend public profiles in parallel
            let entries = [myEntry];
            if (friends.length > 0) {
                const fetches = friends.map(async uid => {
                    try {
                        ...
                        const d = snap.data();
                        // If their weeklyXP is from a past week, treat it as 0
            ...
            // Sort by weeklyXP desc for leaderboard
            ...
            const medals = ['🥇','🥈','🥉'];
            lb.innerHTML = `
                <div style="font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:var(--color-text-secondary);margin-bottom:10px;">This Week</div>
                ${topEntries.map((e, i) => {
                    const rank   = i + 1;
                    const rankBadge = rank <= 3 ? medals[rank-1] : `<span style="font-size:12px;font-weight:700;color:var(--color-text-secondary);min-width:20px;text-align:center;">${rank}</span>`;
                    const isMe   = e.isMe;
                    ...
                    <div onclick="openFriendProfileCard('${escapeHtml(e.uid)}')"
                         style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:${isMe ? 'rgba(93,156,236,0.08)' : 'var(--color-bg-card)'};border:1px solid ${isMe ? 'var(--color-accent-blue)' : 'var(--color-border)'};border-radius:12px;margin-bottom:6px;cursor:pointer;transition:opacity 0.15s;">
            ...
            // ── Add Friend ─────────────────────────────────────────────────
            add.innerHTML = `
                <div style="font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:var(--color-text-secondary);margin-bottom:10px;margin-top:6px;">Add a Friend</div>
                <div style="display:flex;gap:8px;align-items:center;">
                    <input id="friendCodeInput" type="text" maxlength="7" placeholder="MK-XXXX"
                        style="flex:1;background:var(--color-bg-card);border:1px solid var(--color-border);border-radius:10px;padding:10px 14px;font-size:14px;font-weight:600;color:var(--color-text-primary);font-family:inherit;letter-spacing:0.05em;text-transform:uppercase;outline:none;"
                        oninput="this.value=this.value.toUpperCase()" onkeydown="if(event.key==='Enter')addFriendByCode()">
                    <button onclick="addFriendByCode()" style="background:var(--color-accent-blue);color:#fff;border:none;border-radius:10px;padding:10px 18px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;white-space:nowrap;">Add</button>
                </div>
                <div id="friendAddStatus" style="font-size:12px;margin-top:8px;min-height:18px;color:var(--color-text-secondary);"></div>`;

            // ── All Friends list (collapsed) ───────────────────────────────
            if (friends.length === 0) {
                all.innerHTML = `
                    <div style="font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:var(--color-text-secondary);margin-bottom:10px;margin-top:6px;">Friends (0)</div>
                ...
                all.innerHTML = `
                    <div style="font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:var(--color-text-secondary);margin-bottom:10px;margin-top:6px;">Friends (${friendEntries.length})</div>
        ...
        // ── Open read-only friend profile card ───────────────────────────
        window.openFriendProfileCard = function(uid) {
            ...
            // Spider chart values (0–100 scaled to max category)
            const maxXP = Math.max(1, ...cats.map(c => catXP[c.id] || 0));
            const W = 200, H = 200, cx = W/2, cy = H/2, r = 78;
            const angleStep = (2 * Math.PI) / cats.length;
            const polygon = cats.map((c, i) => {
                const pct  = Math.min(1, (catXP[c.id] || 0) / maxXP);
            ...
            document.getElementById('friendProfileContent').innerHTML = `
                ...
                <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:16px;">
                    ${[
                        ...
                        <div style="font-size:14px;font-weight:700;color:var(--color-text-primary);">${t.val}</div>
        ...
        window.copyFriendCode = function() {
            ...
            navigator.clipboard.writeText(code).then(() => {
                ...
                showToast(code, 'blue'); // fallback: just surface it in toast
            });
        };
