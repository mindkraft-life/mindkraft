        ...
        window.renderFriendsTab = async function() {
            ...
            if (friends.length === 0) {
                ...
                all.innerHTML = `
                    <div style="font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:var(--color-text-secondary);margin-bottom:12px;margin-top:24px;">Friends (${friendEntries.length})</div>
        ...
        // ── Deep-link friend add (?add=MK-XXXX) ──────────────────────────
        // Called once on login. If the URL has ?add=CODE, switches to the
        // Friends tab and pre-fills + auto-submits the add flow.
        function handleFriendDeepLink() {
            try {
                ...
                // Switch to Friends tab and kick off add
                switchTab('friends');
                // Wait for tab to render then fill the input
                setTimeout(() => {
                    const input = document.getElementById('friendCodeInput');
                    if (input) {
        ...
        // ── Share your friend code (profile overlay) ──────────────────────
        window.shareFriendCode = async function() {
            ...
            if (navigator.share) {
                try {
                    await navigator.share({ title: 'Add me on Mindkraft', text, url });
                    return;
                } catch(e) {
                    if (e.name === 'AbortError') return; // user cancelled share sheet
                }
            }
            // Fallback: copy the full link
            try {
