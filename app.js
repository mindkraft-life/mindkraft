        ...
        onAuthStateChanged(auth, async (user) => {
            ...
            try {
                if (user) {
                    ...
                    // Ensure public profile exists for friends system (forced, non-blocking)
                    syncPublicProfile(true).catch(e => console.warn('Initial profile sync:', e));
                    // Handle ?addFriend=UID links
        ...
        // ── XP Metric Utilities (Analytics + Friends Leaderboard) ────────

        function getWeekStart() {
            const now = new Date();
            const day = now.getDay(); // 0=Sun — week resets Sunday, matching activity cycles
        ...
        // If filteredActivities is provided (flat array with completionHistory),
        // scope to those. Otherwise scan all activities globally.
        function getXPThisWeek(filteredActivities) {
            const weekStart = getWeekStart();
            let xp = 0;
            if (filteredActivities) {
                filteredActivities.forEach(act => {
                    (act.completionHistory || []).forEach(e => {
                        if (!e.isPenalty && new Date(e.date) >= weekStart) xp += (e.xp || 0);
                    });
                });
            } else {
                (window.userData.dimensions || []).forEach(dim => {
        ...
        function getYesterdayXPMetrics(filteredActivities) {
            ...
            const scanActs = act => {
                (act.completionHistory || []).forEach(e => {
                    if (!e.isPenalty && new Date(e.date).toDateString() === yStr) xp += (e.xp || 0);
                });
            };
            if (filteredActivities) {
                filteredActivities.forEach(scanActs);
            } else {
                (window.userData.dimensions || []).forEach(dim => {
                    (dim.paths || []).forEach(path => {
                        (path.activities || []).forEach(scanActs);
                    });
                });
            }
            return { totalYesterdayXP: xp, xpPerHr: Math.round((xp / 12) * 10) / 10 };
        }
        ...
        function renderAnalyticsSummary(activities, log) {
            ...
            // Scoped to current filter (dimension/path/activity)
            const weeklyXP = getXPThisWeek(activities);
            const { xpPerHr } = getYesterdayXPMetrics(activities);
        ...
        function getMyFriendCode() {
            if (!window.userData.friendCode) window.userData.friendCode = generateFriendCode();
            return window.userData.friendCode;
        }
        ...
        // ── Public Profile Sync ─────────────────────────────────────────
        let _lastPublicProfileSync = 0;
        let _hasInitialSync = false;
        async function syncPublicProfile(force) {
            // Always allow the first sync (on login) to go through
            if (!force && _hasInitialSync && Date.now() - _lastPublicProfileSync < 30000) return;
            _lastPublicProfileSync = Date.now();
            _hasInitialSync = true;
            if (!window.currentUser) return;

            const weeklyXP = getXPThisWeek();
            const { totalYesterdayXP, xpPerHr } = getYesterdayXPMetrics();
            const friendCode = getMyFriendCode();
            ...
            // Category XP for spider chart
            const catXP = getCategoryXP();
            const catXPForProfile = {};
            window.LIFE_CATEGORIES.forEach(c => { catXPForProfile[c.id] = catXP[c.id] || 0; });

            const profile = {
                displayName: (window.userData.profile?.username) || window.currentUser.displayName || 'Anonymous',
                ...
                weeklyXP, yesterdayXP: totalYesterdayXP, xpPerHr, friendCode,
                title: getCharacterTitle(window.userData.level || 1, catXP),
                categoryXP: catXPForProfile,
                bestStreak: allActs.reduce((m, a) => Math.max(m, a.bestStreak || a.streak || 0), 0),
                activeDays: (() => {
                    const ds = new Set();
                    allActs.forEach(a => (a.completionHistory || []).forEach(e => {
                        if (!e.isPenalty && e.date) ds.add(new Date(e.date).toDateString());
                    }));
                    return ds.size;
                })(),
            ...
            await setDoc(doc(db, 'publicProfiles', window.currentUser.uid), profile);
        }

        // ── Add Friend ──────────────────────────────────────────────────
        async function addFriendByUid(friendUid, friendName) {
            if (!window.userData.friendUids) window.userData.friendUids = [];
            if (window.userData.friendUids.includes(friendUid)) return;
            if (window.userData.friendUids.length >= MAX_FRIENDS) return;
            window.userData.friendUids.push(friendUid);
            await saveUserData();
            try {
                await addDoc(collection(db, 'friendRequests'), {
                    fromUid: window.currentUser.uid,
                    fromName: (window.userData.profile?.username) || window.currentUser.displayName || 'Someone',
                    fromPhoto: window.currentUser.photoURL || '',
                    toUid: friendUid,
                    createdAt: new Date().toISOString()
                });
            } catch (e) { console.warn('Friend request write failed:', e); }
            _friendsCacheTime = 0;
            if (window.currentTab === 'friends') renderFriendsTab();
        }

        window.removeFriend = async function(uid) {
            if (!confirm('Remove this friend?')) return;
            if (!window.userData.friendUids) return;
            window.userData.friendUids = window.userData.friendUids.filter(u => u !== uid);
            await saveUserData();
            _friendsCacheTime = 0;
            // Close any open profile overlay
            document.getElementById('friendProfileOverlay')?.remove();
            renderFriendsTab();
        };

        // ── Deep link: ?addFriend=UID ────────────────────────────────────
        function handleAddFriendDeepLink() {
            const params = new URLSearchParams(window.location.search);
            const friendUid = params.get('addFriend');
            if (!friendUid || !window.currentUser) return;
            window.history.replaceState({}, '', window.location.pathname);
            if (friendUid === window.currentUser.uid) return;
            if ((window.userData.friendUids || []).includes(friendUid)) return;
            // Switch to friends tab and show inline add card
            setTimeout(async () => {
                try {
                    const snap = await getDoc(doc(db, 'publicProfiles', friendUid));
                    if (!snap.exists()) return;
                    const data = snap.data();
                    window.switchTab('friends');
                    // Store pending add so renderFriendsTab picks it up
                    window._pendingFriendAdd = { uid: friendUid, profile: data };
                    renderFriendsTab();
                } catch (e) { console.error('Deep link error:', e); }
            }, 600);
        }

        function renderPendingAddCard(container) {
            if (!window._pendingFriendAdd) return;
            const { uid, profile } = window._pendingFriendAdd;
            const card = document.createElement('div');
            card.id = 'pendingAddCard';
            card.className = 'fl-request-card';
            card.style.cssText = 'flex-direction:column;align-items:center;text-align:center;padding:24px 20px;margin-bottom:16px;background:rgba(90,127,212,0.06);';
            card.innerHTML = `
                <div class="fl-lb-avatar-wrap" style="width:56px;height:56px;margin-bottom:12px;">
                    <img src="${escapeHtml(profile.photoURL || '')}" onerror="this.style.display='none'" class="fl-lb-avatar" style="width:56px;height:56px;" alt="">
                    <div class="fl-lb-avatar-fb" style="font-size:22px;">${(profile.displayName || '?')[0]}</div>
                </div>
                <div class="fl-lb-name" style="font-size:16px;justify-content:center;">${escapeHtml(profile.displayName || 'Unknown')}</div>
                <div class="fl-lb-sub" style="margin-bottom:12px;">Lv ${profile.level || 1} \u00b7 ${escapeHtml(profile.title || 'The Initiate')}</div>
                <div style="display:flex;justify-content:center;gap:20px;margin-bottom:14px;">
                    <div style="text-align:center;"><div style="font-size:18px;font-weight:700;color:var(--color-text-primary);">${(profile.totalXP || 0).toLocaleString()}</div><div style="font-size:10px;color:var(--color-text-secondary);text-transform:uppercase;">Total XP</div></div>
                    <div style="text-align:center;"><div style="font-size:18px;font-weight:700;color:var(--color-text-primary);">${profile.bestStreak || 0}</div><div style="font-size:10px;color:var(--color-text-secondary);text-transform:uppercase;">Best Streak</div></div>
                    <div style="text-align:center;"><div style="font-size:18px;font-weight:700;color:var(--color-text-primary);">${profile.activeDays || 0}</div><div style="font-size:10px;color:var(--color-text-secondary);text-transform:uppercase;">Active Days</div></div>
                </div>
                <div style="font-size:13px;color:var(--color-text-secondary);margin-bottom:16px;">wants to connect on Mindkraft</div>
                <div style="display:flex;gap:10px;">
                    <button class="fl-btn-accept" onclick="(async()=>{await addFriendByUid('${uid}','${escapeHtml(profile.displayName||'')}');window._pendingFriendAdd=null;document.getElementById('pendingAddCard')?.remove();})()">Add Friend</button>
                    <button class="fl-btn-skip" onclick="window._pendingFriendAdd=null;document.getElementById('pendingAddCard')?.remove()">Not Now</button>
                </div>`;
        ...
        async function loadFriendsProfiles() {
            const uids = window.userData.friendUids || [];
            if (uids.length === 0) return [];
            if (Date.now() - _friendsCacheTime < 60000 && _friendsCache.length) return _friendsCache;
            const profiles = [];
            for (const uid of uids.slice(0, MAX_FRIENDS)) {
                try {
                    const snap = await getDoc(doc(db, 'publicProfiles', uid));
                    if (snap.exists()) profiles.push({ uid, ...snap.data() });
                } catch (e) { console.warn('Failed to load profile:', uid); }
            }
            _friendsCache = profiles;
        ...
        async function checkPendingFriendRequests() {
            if (!window.currentUser) return;
            try {
                const q = query(collection(db, 'friendRequests'), where('toUid', '==', window.currentUser.uid));
                const snap = await getDocs(q);
                if (snap.empty) return;
                const existing = new Set(window.userData.friendUids || []);
                const pending = [];
                snap.docs.forEach(d => {
                    const data = d.data();
                    if (!existing.has(data.fromUid)) pending.push({ docId: d.id, ...data });
                    else deleteDoc(doc(db, 'friendRequests', d.id)).catch(() => {});
                });
                if (pending.length > 0) renderFriendRequestBanner(pending);
            } catch (e) { console.warn('Friend request check error:', e); }
        }

        function renderFriendRequestBanner(requests) {
            ...
            banner.innerHTML = requests.map(r => `
                <div class="fl-request-card" id="friendReq_${r.docId}">
                    <div class="fl-request-left">
                        <img src="${escapeHtml(r.fromPhoto || '')}" onerror="this.style.display='none'" class="fl-request-avatar" alt="">
                        <div class="fl-request-avatar-fb">${(r.fromName || '?')[0]}</div>
                    </div>
                    <div class="fl-request-info">
                        <div class="fl-request-name">${escapeHtml(r.fromName || 'Someone')}</div>
                        <div class="fl-request-msg">added you as a friend</div>
                    </div>
                    <div class="fl-request-actions">
                        <button class="fl-btn-accept-sm" onclick="acceptFriendRequest('${r.docId}','${r.fromUid}','${escapeHtml(r.fromName || '')}')">Add back</button>
                        <button class="fl-btn-skip-sm" onclick="dismissFriendRequest('${r.docId}')">Skip</button>
                    </div>
                </div>
            `).join('');
            const container = document.getElementById('friendsContent') || document.getElementById('friendsSubMyFriends');
            if (container) container.prepend(banner);
        }

        window.acceptFriendRequest = async function(docId, fromUid, fromName) {
            const el = document.getElementById('friendReq_' + docId);
            if (el) el.style.opacity = '0.5';
            if (!window.userData.friendUids) window.userData.friendUids = [];
            if (!window.userData.friendUids.includes(fromUid) && window.userData.friendUids.length < MAX_FRIENDS) {
                window.userData.friendUids.push(fromUid);
                await saveUserData();
            }
            try { await deleteDoc(doc(db, 'friendRequests', docId)); } catch (e) {}
            if (el) { el.innerHTML = `<div style="padding:14px;color:var(--color-accent-green);font-size:13px;font-weight:600;text-align:center;">\u2713 Added ${fromName || 'friend'}!</div>`; el.style.opacity = '1'; }
            _friendsCacheTime = 0;
            setTimeout(() => { if (el) el.remove(); renderFriendsTab(); }, 1500);
        };
        ...
        // ── Spider SVG Generator ────────────────────────────────────────
        function generateSpiderSVG(categoryXP, size) {
            const cats = window.LIFE_CATEGORIES;
            const S = size || 200;
            const cx = S / 2, cy = S / 2, R = S * 0.36;
            const N = cats.length;
            const angle = i => (Math.PI * 2 * i / N) - Math.PI / 2;
            const maxXP = Math.max(...cats.map(c => categoryXP[c.id] || 0), 1);

            let svg = `<svg viewBox="0 0 ${S} ${S}" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;max-width:${S}px;margin:0 auto;">`;
            ...
            [0.25, 0.5, 0.75, 1].forEach(frac => {
                ...
                d += 'Z';
                svg += `<path d="${d}" fill="none" stroke="rgba(255,255,255,${frac === 1 ? 0.12 : 0.05})" stroke-width="0.8"/>`;
            });

            // Spokes
            cats.forEach((_, i) => {
                svg += `<line x1="${cx}" y1="${cy}" x2="${cx + R * Math.cos(angle(i))}" y2="${cy + R * Math.sin(angle(i))}" stroke="rgba(255,255,255,0.07)" stroke-width="0.8"/>`;
            });

            // Data polygon
            let dataD = '';
            cats.forEach((c, i) => {
                const frac = Math.max(0.05, (categoryXP[c.id] || 0) / maxXP);
                const r = R * frac;
                const x = cx + r * Math.cos(angle(i));
                const y = cy + r * Math.sin(angle(i));
                dataD += i === 0 ? `M${x},${y}` : `L${x},${y}`;
            });
            dataD += 'Z';
            svg += `<path d="${dataD}" fill="rgba(90,127,212,0.2)" stroke="rgba(90,127,212,0.7)" stroke-width="1.5"/>`;
            ...
            cats.forEach((c, i) => {
                ...
                const x = cx + r * Math.cos(angle(i));
                const y = cy + r * Math.sin(angle(i));
                svg += `<circle cx="${x}" cy="${y}" r="2.5" fill="${c.color}" stroke="#fff" stroke-width="0.5"/>`;
            });
            ...
            cats.forEach((c, i) => {
                ...
                svg += `<text x="${x}" y="${y + 4}" text-anchor="${anchor}" fill="${c.color}" font-size="10" font-weight="600" font-family="inherit">${c.label}</text>`;
            });
        ...
        // ── Friends Tab Render ───────────────────────────────────────────
        window._friendsLeaderboardMode = 'weekly';
        window._friendsListExpanded = false;

        window.setFriendsLeaderboardMode = function(mode, btn) {
            window._friendsLeaderboardMode = mode;
            btn.closest('.fl-toggle-pills').querySelectorAll('.fl-toggle-pill').forEach(p => p.classList.remove('fl-toggle-active'));
            btn.classList.add('fl-toggle-active');
            renderFriendsLeaderboard();
        };

        window.toggleFriendsList = function() {
            window._friendsListExpanded = !window._friendsListExpanded;
            const body = document.getElementById('friendsListBody');
            const chevron = document.getElementById('friendsListChevron');
            if (body) body.style.display = window._friendsListExpanded ? '' : 'none';
            if (chevron) chevron.style.transform = window._friendsListExpanded ? 'rotate(180deg)' : '';
        };

        async function renderFriendsTab() {
            const container = document.getElementById('friendsSubMyFriends');
            if (!container) return;
            const uids = window.userData.friendUids || [];
            const friendCount = uids.length;

            // Add-friend inline row
            const addSection = friendCount >= MAX_FRIENDS
                ? `<div style="font-size:12px;color:var(--color-text-secondary);text-align:center;padding:10px 0;">Friend limit reached (${MAX_FRIENDS})</div>`
                : `<div class="fl-add-row">
                    <input type="text" id="inlineAddFriendCode" placeholder="MK-XXXX" maxlength="7"
                        class="fl-add-input" onkeydown="if(event.key==='Enter')submitInlineAddFriend()">
                    <button class="fl-add-btn" onclick="submitInlineAddFriend()">Add</button>
                </div>
                <div id="inlineAddFriendStatus" class="fl-add-status"></div>`;
                ...
                container.innerHTML = `
                    <div class="fl-container">
                        <div class="fl-empty">
                            ...
                            <div class="fl-empty-title">No friends yet</div>
                            <div class="fl-empty-sub">Share your friend code from your Profile, or add someone with theirs.</div>
                        </div>
                        <div class="fl-section-label">ADD A FRIEND</div>
                        ${addSection}
                    </div>`;
            ...
            container.innerHTML = '<div class="fl-loading">Loading\u2026</div>';

            const friends = await loadFriendsProfiles();
            const weeklyXP = getXPThisWeek();
            const { xpPerHr } = getYesterdayXPMetrics();
            const catXP = getCategoryXP();
            const me = {
                uid: window.currentUser.uid,
                displayName: (window.userData.profile?.username) || window.currentUser.displayName || 'You',
                photoURL: window.currentUser.photoURL || '',
                level: window.userData.level || 1,
                title: getCharacterTitle(window.userData.level || 1, catXP),
                totalXP: (window.userData.totalXP || 0) + (window.userData.xpDeletedGhost || 0),
                weeklyXP, xpPerHr,
                categoryXP: (() => { const m = {}; window.LIFE_CATEGORIES.forEach(c => { m[c.id] = catXP[c.id] || 0; }); return m; })(),
                bestStreak: (() => { let mx = 0; (window.userData.dimensions||[]).forEach(d=>(d.paths||[]).forEach(p=>(p.activities||[]).forEach(a=>{mx=Math.max(mx,a.bestStreak||a.streak||0);}))); return mx; })(),
                activeDays: (() => { const ds = new Set(); (window.userData.dimensions||[]).forEach(d=>(d.paths||[]).forEach(p=>(p.activities||[]).forEach(a=>(a.completionHistory||[]).forEach(e=>{if(!e.isPenalty&&e.date)ds.add(new Date(e.date).toDateString());})))); return ds.size; })(),
                isMe: true
            };
            window._friendsLeaderboardData = [me, ...friends];

            container.innerHTML = `
                <div class="fl-container" id="friendsContent">
                    <!-- Leaderboard -->
                    <div class="fl-section-label" style="display:flex;align-items:center;justify-content:space-between;">
                        <span>${window._friendsLeaderboardMode === 'weekly' ? 'THIS WEEK' : 'YESTERDAY'}</span>
                        <div class="fl-toggle-pills">
                            <button class="fl-toggle-pill ${window._friendsLeaderboardMode === 'weekly' ? 'fl-toggle-active' : ''}" onclick="setFriendsLeaderboardMode('weekly',this)">Weekly</button>
                            <button class="fl-toggle-pill ${window._friendsLeaderboardMode === 'xphr' ? 'fl-toggle-active' : ''}" onclick="setFriendsLeaderboardMode('xphr',this)">XP/Hr</button>
                        </div>
                    </div>
                    <div id="friendsLeaderboardContent"></div>
                    ...
                    <!-- All Friends -->
                    <div class="fl-section-label" onclick="toggleFriendsList()" style="cursor:pointer;display:flex;align-items:center;justify-content:space-between;">
                        <span>ALL FRIENDS (${friends.length})</span>
                        <span id="friendsListChevron" style="font-size:10px;transition:transform 0.2s;${window._friendsListExpanded ? 'transform:rotate(180deg);' : ''}">\u25BC</span>
                    </div>
                    <div id="friendsListBody" style="display:${window._friendsListExpanded ? '' : 'none'};">
                        ${friends.map(f => `
                            <div class="fl-list-card" onclick="openFriendProfile('${f.uid}')">
                                <div class="fl-list-avatar-wrap">
                                    <img src="${escapeHtml(f.photoURL || '')}" onerror="this.style.display='none'" class="fl-list-avatar" alt="">
                                    <div class="fl-list-avatar-fb">${(f.displayName || '?')[0]}</div>
                                </div>
                                <div class="fl-list-info">
                                    <div class="fl-list-name">${escapeHtml(f.displayName || 'Unknown')}</div>
                                    <div class="fl-list-sub">Lv ${f.level || 1} \u00b7 ${escapeHtml(f.title || 'The Initiate')}</div>
                                </div>
                                <span class="fl-list-chevron">\u203A</span>
                            </div>
                        `).join('')}
                    </div>
                </div>`;
            ...
            renderFriendsLeaderboard();
            // Inline cards for pending adds and friend requests (above leaderboard)
            const contentEl = document.getElementById('friendsContent');
            if (contentEl) renderPendingAddCard(contentEl);
            checkPendingFriendRequests();
        }

        function renderFriendsLeaderboard() {
            const el = document.getElementById('friendsLeaderboardContent');
            if (!el) return;
            const data = window._friendsLeaderboardData || [];
            if (data.length === 0) { el.innerHTML = '<div class="fl-loading">No data</div>'; return; }

            const isWeekly = window._friendsLeaderboardMode === 'weekly';
            const sorted = [...data].sort((a, b) => (isWeekly ? (b.weeklyXP||0) : (b.xpPerHr||0)) - (isWeekly ? (a.weeklyXP||0) : (a.xpPerHr||0)));
            const medals = ['\ud83e\udd47', '\ud83e\udd48', '\ud83e\udd49'];

            // Update section label
            const labelEl = el.closest('.fl-container')?.querySelector('.fl-section-label span');
            if (labelEl) labelEl.textContent = isWeekly ? 'THIS WEEK' : 'YESTERDAY';

            el.innerHTML = sorted.map((p, i) => {
                const val = isWeekly ? (p.weeklyXP || 0) : (p.xpPerHr || 0);
                const valLabel = isWeekly ? 'XP this week' : 'XP/hr';
                const meClass = p.isMe ? ' fl-lb-me' : '';
                const youBadge = p.isMe ? '<span class="fl-lb-you">YOU</span>' : '';
                const clickAttr = ` onclick="openFriendProfile('${p.uid}')"`;
                return `
                    <div class="fl-lb-card${meClass}" style="cursor:pointer;"${clickAttr}>
                        <span class="fl-lb-medal">${i < 3 ? medals[i] : ''}</span>
                        <div class="fl-lb-avatar-wrap">
                            <img src="${escapeHtml(p.photoURL || '')}" onerror="this.style.display='none'" class="fl-lb-avatar" alt="">
                            <div class="fl-lb-avatar-fb">${(p.displayName || '?')[0]}</div>
                        </div>
                        <div class="fl-lb-info">
                            <div class="fl-lb-name">${escapeHtml(p.displayName || 'Unknown')} ${youBadge}</div>
                            <div class="fl-lb-sub">Lv ${p.level || 1} \u00b7 ${escapeHtml(p.title || 'The Initiate')}</div>
                        </div>
                        <div class="fl-lb-xp">
                            <div class="fl-lb-xp-val">${val.toLocaleString()}</div>
                            <div class="fl-lb-xp-label">${valLabel}</div>
                        </div>
                    </div>`;
            }).join('');
        }

        window.submitInlineAddFriend = async function() {
            const input = document.getElementById('inlineAddFriendCode');
            const status = document.getElementById('inlineAddFriendStatus');
            if (!input || !status) return;
            const raw = input.value.replace(/[\s\-]/g, '').trim().toUpperCase();
            if (!raw || raw.length < 4) { status.textContent = 'Code is too short.'; status.className = 'fl-add-status fl-add-error'; return; }
            if (raw === getMyFriendCode()) { status.textContent = "That's your own code!"; status.className = 'fl-add-status fl-add-error'; return; }

            status.textContent = 'Looking up\u2026'; status.className = 'fl-add-status';
            try {
                const q = query(collection(db, 'publicProfiles'), where('friendCode', '==', raw));
                const snap = await getDocs(q);
                if (snap.empty) { status.textContent = 'No user found \u2014 ask them to open Mindkraft first.'; status.className = 'fl-add-status fl-add-error'; return; }
                const friendDoc = snap.docs[0];
                const friendUid = friendDoc.id;
                if (friendUid === window.currentUser.uid) { status.textContent = "That's you!"; status.className = 'fl-add-status fl-add-error'; return; }
                if ((window.userData.friendUids || []).includes(friendUid)) { status.textContent = 'Already friends!'; status.className = 'fl-add-status'; return; }
                const data = friendDoc.data();
                await addFriendByUid(friendUid, data.displayName || 'Unknown');
                input.value = '';
                status.textContent = '\u2713 Added ' + (data.displayName || 'friend') + '!';
                status.className = 'fl-add-status fl-add-success';
            } catch (e) {
                console.error('Add friend error:', e);
                status.textContent = 'Error. Try again.'; status.className = 'fl-add-status fl-add-error';
            }
        };

        // ── Friend Profile Overlay ──────────────────────────────────────
        window.openFriendProfile = function(uid) {
            const friends = window._friendsLeaderboardData || [];
            const f = friends.find(p => p.uid === uid);
            if (!f) return;

            const existing = document.getElementById('friendProfileOverlay');
            if (existing) existing.remove();

            const catXP = f.categoryXP || {};
            const spiderSVG = generateSpiderSVG(catXP, 220);
            const hasCatData = Object.values(catXP).some(v => v > 0);

            const overlay = document.createElement('div');
            overlay.id = 'friendProfileOverlay';
            overlay.style.cssText = 'position:fixed;inset:0;z-index:9000;background:var(--color-bg-primary);overflow-y:auto;animation:fl-slideUp 0.3s ease;';
            overlay.innerHTML = `
                <div style="max-width:480px;margin:0 auto;padding:0 0 80px;">
                    <!-- Header -->
                    <div class="fl-profile-header">
                        <span>PROFILE</span>
                        <button onclick="document.getElementById('friendProfileOverlay').remove()" class="fl-profile-close">
                    ...
                    <div style="padding:20px;">
                        <!-- Identity -->
                        <div class="fl-profile-identity">
                            <div class="fl-profile-avatar-wrap">
                                <img src="${escapeHtml(f.photoURL || '')}" onerror="this.style.display='none'" class="fl-profile-avatar" alt="">
                                <div class="fl-profile-avatar-fb">${(f.displayName || '?')[0]}</div>
                            </div>
                            <div class="fl-profile-title">${escapeHtml(f.title || 'The Initiate')}</div>
                            <div class="fl-profile-name">${escapeHtml(f.displayName || 'Unknown')}</div>
                            <div class="fl-profile-level">Level ${f.level || 1}</div>
                        </div>

                        <!-- Stats Grid -->
                        <div class="fl-profile-stats">
                            <div class="fl-profile-stat" style="border-left-color:var(--color-accent-blue);">
                                <div class="fl-profile-stat-val">${(f.totalXP || 0).toLocaleString()}</div>
                                <div class="fl-profile-stat-lbl">Total XP</div>
                            </div>
                            <div class="fl-profile-stat" style="border-left-color:var(--color-accent-green);">
                                <div class="fl-profile-stat-val">${(f.weeklyXP || 0).toLocaleString()}</div>
                                <div class="fl-profile-stat-lbl">This Week</div>
                            </div>
                            <div class="fl-profile-stat" style="border-left-color:#e05c3a;">
                                <div class="fl-profile-stat-val">${f.bestStreak || 0}</div>
                                <div class="fl-profile-stat-lbl">Best Streak</div>
                            </div>
                            <div class="fl-profile-stat" style="border-left-color:#d4a03a;">
                                <div class="fl-profile-stat-val">${f.activeDays || 0}</div>
                                <div class="fl-profile-stat-lbl">Active Days</div>
                            </div>
                            <div class="fl-profile-stat" style="border-left-color:#d45a9f;">
                                <div class="fl-profile-stat-val">${f.level || 1}</div>
                                <div class="fl-profile-stat-lbl">Level</div>
                            </div>
                            <div class="fl-profile-stat" style="border-left-color:#6b9e5e;">
                                <div class="fl-profile-stat-val" style="font-size:14px;">${escapeHtml(f.title || 'The Initiate')}</div>
                                <div class="fl-profile-stat-lbl">Title</div>
                            </div>
                        </div>

                        <!-- Spider Chart -->
                        ${hasCatData ? `
                            <div class="fl-profile-spider">
                                ${spiderSVG}
                            </div>
                            <div class="fl-profile-legend">
                                ${window.LIFE_CATEGORIES.map(c => `<span class="fl-legend-dot" style="color:${c.color};">\u25CF ${c.label}</span>`).join(' ')}
                            </div>
                        ` : ''}

                        <!-- Remove / Self indicator -->
                        ${f.isMe ? `
                            <div style="text-align:center;padding:12px;font-size:12px;color:var(--color-text-secondary);border:1px solid var(--color-border);border-radius:10px;margin-top:8px;">
                                This is how your profile appears to friends
                            </div>
                        ` : `
                            <button class="fl-profile-remove" onclick="removeFriend('${f.uid}')">Remove Friend</button>
                        `}
                    </div>
                </div>`;
            document.body.appendChild(overlay);
        ...
        // ── Profile Overlay: Friend Code ────────────────────────────────
        function renderFriendCodeInProfile() {
            const anchor = document.getElementById('profileMemberSince');
            if (!anchor) return;
            const old = document.getElementById('profileFriendCodeRow');
            if (old) old.remove();

            const code = getMyFriendCode();
            const display = formatFriendCode(code);
            const shareURL = window.location.origin + window.location.pathname + '?addFriend=' + (window.currentUser?.uid || '');

            const row = document.createElement('div');
            row.id = 'profileFriendCodeRow';
            row.style.cssText = 'display:flex;align-items:center;gap:6px;margin-top:8px;';
            row.innerHTML = `
                <span style="font-size:10px;color:var(--color-text-secondary);font-weight:600;text-transform:uppercase;letter-spacing:0.06em;">Code</span>
                <span style="font-size:13px;font-weight:700;font-family:monospace;letter-spacing:0.08em;color:var(--color-text-primary);background:var(--color-bg-card);border:1px solid var(--color-border);padding:2px 8px;border-radius:6px;">${display}</span>
                <button onclick="navigator.clipboard.writeText('${display}');this.textContent='Copied!';setTimeout(()=>this.textContent='Copy',1200)" style="background:none;border:1px solid var(--color-border);color:var(--color-text-secondary);padding:2px 8px;border-radius:6px;font-size:10px;font-weight:600;cursor:pointer;font-family:inherit;">Copy</button>
                <button onclick="(navigator.share?navigator.share({title:'Add me on Mindkraft!',text:'My code: ${display}',url:'${shareURL}'}):navigator.clipboard.writeText('${shareURL}')).then(()=>{}).catch(()=>{});this.textContent=navigator.share?'Shared!':'Copied!';setTimeout(()=>this.textContent='\ud83d\udce4 Share',1200)" style="background:none;border:1px solid var(--color-border);color:var(--color-text-secondary);padding:2px 8px;border-radius:6px;font-size:10px;font-weight:600;cursor:pointer;font-family:inherit;">\ud83d\udce4 Share</button>`;
            anchor.after(row);
        }
        ...
        async function saveUserData() {
            ...
            try {
                ...
                // Sync public profile for friends leaderboard (throttled, non-blocking)
                syncPublicProfile().catch(e => console.warn('publicProfile sync:', e));
            } catch (error) {
        ...
        // ── Swipe Navigation for Sub-Tabs ──────────────────────────────
        // Maps each parent tab to its ordered sub-tab names
        const _swipeSubTabs = {
            activities: ['planner', 'myActivities', 'categories'],
            challenges: ['myChallenges', 'publicChallenges'],
            friends: ['myFriends', 'groupChallenges'],
        };

        function initSwipeNavigation() {
            Object.keys(_swipeSubTabs).forEach(parentTab => {
                const tabEl = document.getElementById(parentTab + 'Tab');
                if (!tabEl) return;
                ...
                tabEl.addEventListener('touchend', function(e) {
                    ...
                    const subs = _swipeSubTabs[parentTab];
                    // Find current active sub-tab
                    const pills = tabEl.querySelectorAll('.sub-tab');
                    let currentIdx = 0;
                    pills.forEach((btn, i) => { if (btn.classList.contains('active')) currentIdx = i; });

                    const nextIdx = dx < 0 ? currentIdx + 1 : currentIdx - 1; // swipe left = next
                    if (nextIdx >= 0 && nextIdx < subs.length) {
                        window.switchSubTab(parentTab, subs[nextIdx]);
                    }
                }, { passive: true });
            });
        }

        // Init swipe after DOM is ready
        ...
        function renderProfileOverlay() {
            ...
            // ── Friend code row under identity card ──────────────────────
            try { renderFriendCodeInProfile(); } catch(e) { console.warn('renderFriendCodeInProfile', e); }
