        ...
        async function renderFriendsTab() {
            ...
            const addRow = friendCount >= MAX_FRIENDS
                ...
                : `<div style="display:flex;gap:8px;align-items:center;">
                    <input type="text" id="inlineAddFriendCode" class="text-input" placeholder="Enter friend code" maxlength="6"
        ...
        function renderFriendCodeInProfile() {
            ...
            row.innerHTML = `
                <span style="font-size:11px;color:var(--color-text-secondary);font-weight:600;text-transform:uppercase;letter-spacing:0.06em;">Friend Code</span>
                <span style="font-size:15px;font-weight:700;font-family:monospace;letter-spacing:0.12em;color:var(--color-text-primary);background:var(--color-bg-card);border:1px solid var(--color-border);padding:4px 10px;border-radius:8px;">${code}</span>
                <button onclick="navigator.clipboard.writeText('${code}');this.textContent='Copied!';setTimeout(()=>this.textContent='Copy',1500)" style="background:none;border:1px solid var(--color-border);color:var(--color-text-secondary);padding:4px 10px;border-radius:8px;font-size:11px;font-weight:600;cursor:pointer;font-family:inherit;">Copy</button>
                <button onclick="(navigator.share?navigator.share({title:'Add me on Mindkraft!',text:'My friend code: ${code}',url:'${shareURL}'}):navigator.clipboard.writeText('${shareURL}')).then(()=>{}).catch(()=>{});this.textContent=navigator.share?'Shared!':'Link copied!';setTimeout(()=>this.textContent='\ud83d\udce4 Share',1500)" style="background:none;border:1px solid var(--color-border);color:var(--color-text-secondary);padding:4px 10px;border-radius:8px;font-size:11px;font-weight:600;cursor:pointer;font-family:inherit;">\ud83d\udce4 Share</button>
            `;
            anchor.after(row);
        }
