        ...
        // ══════════════════════════════════════════════════════════════════
        // ── Friends System ───────────────────────────────────────────────
        // WRITES: syncPublicProfile() on every save (throttled 30s)
        // READS:  All deferred to Friends tab open
        // ADD:    Short 6-char code (manual) or share link (auto)
        // CAP:    Max 20 friends
        // ══════════════════════════════════════════════════════════════════
        ...
        // ── Public Profile Sync (write-side) ────────────────────────────
        ...
        async function syncPublicProfile() {
            ...
            const profile = {
                displayName: window.currentUser.displayName || (window.userData.profile?.username) || 'Anonymous',
            ...
            const profileRef = doc(db, 'publicProfiles', window.currentUser.uid);
            await setDoc(profileRef, profile);
        }

        // ── Add Friend (inline in Friends tab) ────────────────────────────
        ...
        async function addFriendByUid(friendUid, friendName) {
            ...
            try {
                await addDoc(collection(db, 'friendRequests'), {
                    ...
                    fromName: window.currentUser.displayName || (window.userData.profile?.username) || 'Someone',
        ...
        function handleAddFriendDeepLink() {
            ...
            setTimeout(async () => {
                try {
                    ...
                    if (!snap.exists()) { alert('Friend not found.'); return; }
                    const data = snap.data();
                    if (confirm('Add ' + (data.displayName || 'this user') + ' as a friend?')) {
                        await addFriendByUid(friendUid, data.displayName || 'Unknown');
                        alert('\u2713 Added ' + (data.displayName || 'friend') + '!');
                    }
        ...
        // ── Friend Request Check (on tab open) ──────────────────────────
        async function checkPendingFriendRequests() {
            ...
            try {
                ...
                if (pending.length > 0) showFriendRequestPrompt(pending);
            } catch (e) { console.warn('Friend request check error:', e); }
        }

        function showFriendRequestPrompt(requests) {
            ...
            modal.innerHTML = `
                <div class="modal-content" style="max-width:400px;">
                    <div class="modal-header">
                        ...
                        <button class="modal-close" onclick="document.getElementById('friendRequestModal').remove()">\u2715</button>
                    </div>
                    <div class="modal-body" style="padding:16px;">
                        <div style="font-size:13px;color:var(--color-text-secondary);margin-bottom:14px;">
                            ${requests.length} ${requests.length === 1 ? 'person has' : 'people have'} added you!
                        </div>
                        <div id="friendRequestList">
                            ${requests.map(r => `
                                <div class="friend-card" id="friendReq_${r.docId}">
                                    <img class="friend-avatar" src="${escapeHtml(r.fromPhoto || '')}" onerror="this.style.display='none'" alt="">
                                    <div class="friend-info">
                                        <div class="friend-name">${escapeHtml(r.fromName || 'Someone')}</div>
                                    </div>
                                    <div style="display:flex;gap:6px;">
                                        <button class="filter-pill" style="background:var(--color-accent-green);color:#fff;border:none;" onclick="acceptFriendRequest('${r.docId}','${r.fromUid}','${escapeHtml(r.fromName || '')}')">Add back</button>
                                        <button class="filter-pill" onclick="dismissFriendRequest('${r.docId}')">Skip</button>
                                    </div>
                                </div>
                            `).join('')}
                        </div>
        ...
        window.acceptFriendRequest = async function(docId, fromUid, fromName) {
            ...
            if (el) { el.innerHTML = `<div style="padding:10px;color:var(--color-accent-green);font-size:13px;">\u2713 Added ${fromName || 'friend'}!</div>`; el.style.opacity = '1'; }
            _friendsCacheTime = 0;
            setTimeout(() => {
                const left = document.querySelectorAll('#friendRequestList .friend-card button');
                if (left.length === 0) document.getElementById('friendRequestModal')?.remove();
            }, 1200);
        };
        ...
        window.dismissFriendRequest = async function(docId) {
            ...
            const el = document.getElementById('friendReq_' + docId);
            if (el) el.remove();
            if (!document.querySelectorAll('#friendRequestList .friend-card').length) document.getElementById('friendRequestModal')?.remove();
        };
        ...
        window.toggleFriendsList = function() {
            ...
            if (chevron) chevron.textContent = window._friendsListExpanded ? '\u25B2' : '\u25BC';
        };
        ...
        async function renderFriendsTab() {
            ...
            // Inline add-friend row (used in both empty and populated states)
            const addRow = friendCount >= MAX_FRIENDS
                ? `<div style="font-size:12px;color:var(--color-text-secondary);text-align:center;padding:8px 0;">Friend limit reached (${MAX_FRIENDS})</div>`
                : `<div style="display:flex;gap:8px;align-items:center;">
                    <input type="text" id="inlineAddFriendCode" placeholder="Enter code" maxlength="7"
                        style="flex:1;font-size:15px;font-family:monospace;text-transform:uppercase;letter-spacing:0.1em;
                        background:var(--color-bg-card);border:1px solid var(--color-border);border-radius:10px;
                        padding:10px 14px;color:var(--color-text-primary);outline:none;font-family:monospace;"
                        onfocus="this.style.borderColor='var(--color-accent-blue)'"
                        onblur="this.style.borderColor='var(--color-border)'"
                        onkeydown="if(event.key==='Enter')submitInlineAddFriend()">
                    <button class="btn-primary" onclick="submitInlineAddFriend()" style="white-space:nowrap;padding:10px 18px;">Add</button>
                </div>
                <div id="inlineAddFriendStatus" style="font-size:12px;min-height:18px;margin-top:6px;color:var(--color-text-secondary);"></div>`;

            // Empty state
            if (uids.length === 0) {
                container.innerHTML = `
                    <div style="padding:20px;">
                        <div class="empty-placeholder" style="margin-bottom:20px;">
                            <div style="font-size:36px;margin-bottom:12px;">👥</div>
                            <div style="font-size:15px;font-weight:600;color:var(--color-text-primary);margin-bottom:6px;">No friends yet</div>
                            <div style="font-size:13px;color:var(--color-text-secondary);line-height:1.5;">Share your friend code from your Profile, or add someone using theirs.</div>
                        </div>
                        ${addRow}
                    </div>`;
            ...
            // Loading
            container.innerHTML = '<div style="text-align:center;padding:40px 0;color:var(--color-text-secondary);">Loading\u2026</div>';
            ...
            container.innerHTML = `
                <div style="padding:16px;">
                    ...
                    <div class="analytics-card" style="margin-bottom:16px;">
                        <div class="analytics-card-header" style="flex-wrap:wrap;gap:8px;">
                            <h3 class="analytics-card-title">\ud83c\udfc6 Leaderboard</h3>
                            <div class="filter-pills" style="gap:4px;">
                                <button class="filter-pill ${window._friendsLeaderboardMode === 'weekly' ? 'active' : ''}" onclick="setFriendsLeaderboardMode('weekly',this)">This Week</button>
                                <button class="filter-pill ${window._friendsLeaderboardMode === 'xphr' ? 'active' : ''}" onclick="setFriendsLeaderboardMode('xphr',this)">XP/Hr</button>
                            </div>
                        </div>
                    ...
                    <button onclick="toggleFriendsList()" style="width:100%;display:flex;align-items:center;gap:10px;background:var(--color-bg-card);border:1px solid var(--color-border);border-radius:12px;padding:12px 16px;cursor:pointer;font-family:inherit;color:var(--color-text-primary);text-align:left;">
                        <span style="font-size:14px;font-weight:600;flex:1;">Friends (${friends.length}/${MAX_FRIENDS})</span>
                        <span id="friendsListChevron" style="font-size:11px;color:var(--color-text-secondary);">${window._friendsListExpanded ? '\u25B2' : '\u25BC'}</span>
                    </button>
                    <div id="friendsListBody" style="display:${window._friendsListExpanded ? '' : 'none'};padding-top:10px;">
                        ${friends.map(f => `
                            <div class="friend-card" onclick="openFriendProfile('${f.uid}')" style="cursor:pointer;">
                                <img class="friend-avatar" src="${escapeHtml(f.photoURL || '')}" onerror="this.style.display='none'" alt="">
                                <div class="friend-info">
                                    <div class="friend-name">${escapeHtml(f.displayName || 'Unknown')}</div>
                                    <div class="friend-meta">Lv ${f.level || 1} \u00b7 ${(f.weeklyXP || 0).toLocaleString()} XP this week</div>
                                </div>
        ...
        window.submitInlineAddFriend = async function() {
            ...
            // Strip hyphens/spaces — stored codes are 6 chars, displayed as XXX-XXX
            const raw = input.value.replace(/[\s\-]/g, '').trim().toUpperCase();
            if (!raw || raw.length < 4) { status.textContent = 'Code is too short.'; status.style.color = 'var(--color-accent-red)'; return; }
            if (raw === getMyFriendCode()) { status.textContent = "That's your own code!"; status.style.color = 'var(--color-accent-red)'; return; }
            ...
            try {
                ...
                const snap = await getDocs(q);
                if (snap.empty) { status.textContent = 'No user found.'; status.style.color = 'var(--color-accent-red)'; return; }
                const friendDoc = snap.docs[0];
                const friendUid = friendDoc.id;
                if (friendUid === window.currentUser.uid) { status.textContent = "That's you!"; status.style.color = 'var(--color-accent-red)'; return; }
                if ((window.userData.friendUids || []).includes(friendUid)) { status.textContent = 'Already friends!'; status.style.color = 'var(--color-text-secondary)'; return; }
        ...
        function renderFriendsLeaderboard() {
            ...
            if (data.length === 0) { el.innerHTML = '<div style="padding:16px 0;text-align:center;color:var(--color-text-secondary);">No data</div>'; return; }

            const isWeekly = window._friendsLeaderboardMode === 'weekly';
            const sorted = [...data].sort((a, b) => {
                return (isWeekly ? (b.weeklyXP || 0) : (b.xpPerHr || 0)) - (isWeekly ? (a.weeklyXP || 0) : (a.xpPerHr || 0));
            });
            const max = (isWeekly ? (sorted[0].weeklyXP || 0) : (sorted[0].xpPerHr || 0)) || 1;
            const medals = ['\ud83e\udd47', '\ud83e\udd48', '\ud83e\udd49'];
            const colors = ['#d4a03a', '#9e9e9e', '#cd7f32', '#5a9fd4', '#6b7c3f'];

            el.innerHTML = sorted.map((p, i) => {
                const val = isWeekly ? (p.weeklyXP || 0) : (p.xpPerHr || 0);
                const label = isWeekly ? val.toLocaleString() + ' XP' : val + ' XP/hr';
                const meStyle = p.isMe ? 'background:var(--color-bg-hover);border-radius:8px;' : '';
                const nameStyle = p.isMe ? 'font-weight:700;color:var(--color-accent-blue);' : '';
                const clickAttr = p.isMe ? '' : ` onclick="openFriendProfile('${p.uid}')" style="cursor:pointer;${meStyle}"`;
                return `<div class="rank-row"${clickAttr}${p.isMe ? ` style="${meStyle}"` : ''}>
                    <span class="rank-num">${i < 3 ? medals[i] : '#' + (i + 1)}</span>
                    <span class="rank-label" style="${nameStyle}">${escapeHtml(p.displayName || 'Unknown')}</span>
                    <div class="rank-bar-track">
                        <div class="rank-bar-fill" style="width:${(val / max * 100).toFixed(1)}%;background:${colors[i % colors.length]};"></div>
                    </div>
        ...
        window.openFriendProfile = function(uid) {
            ...
            modal.innerHTML = `
                <div class="modal-content" style="max-width:380px;">
                    <div class="modal-header">
                        <h3 class="modal-title">${escapeHtml(f.displayName || 'Unknown')}</h3>
                        <button class="modal-close" onclick="document.getElementById('friendProfileModal').remove()">\u2715</button>
                    </div>
                    <div class="modal-body" style="padding:20px;">
                        <div style="display:flex;align-items:center;gap:14px;margin-bottom:20px;">
                            <img src="${escapeHtml(f.photoURL || '')}" onerror="this.style.display='none'" style="width:52px;height:52px;border-radius:50%;object-fit:cover;background:var(--color-bg-hover);" alt="">
                            <div>
                                <div style="font-size:18px;font-weight:700;color:var(--color-text-primary);">Level ${f.level || 1}</div>
                                <div style="font-size:12px;color:var(--color-text-secondary);">${(f.totalXP || 0).toLocaleString()} total XP</div>
                            </div>
                        </div>
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px;">
                            <div class="analytics-stat">
                                <div class="analytics-stat-value">${(f.weeklyXP || 0).toLocaleString()}</div>
                                <div class="analytics-stat-label">XP This Week</div>
                            </div>
                            <div class="analytics-stat">
                                <div class="analytics-stat-value">${f.xpPerHr || 0}</div>
                                <div class="analytics-stat-label">XP/Hr Yesterday</div>
                            </div>
                            <div class="analytics-stat">
                                <div class="analytics-stat-value">${f.bestStreak || 0}</div>
                                <div class="analytics-stat-label">Best Streak</div>
                            </div>
                            <div class="analytics-stat">
                                <div class="analytics-stat-value">${f.activeDays || 0}</div>
                                <div class="analytics-stat-label">Active Days</div>
                            </div>
                        </div>
                        <div style="text-align:center;">
                            <button class="filter-pill" style="color:var(--color-accent-red);" onclick="removeFriend('${f.uid}');document.getElementById('friendProfileModal')?.remove();">Remove friend</button>
                        </div>
                    </div>
                </div>`;
            document.body.appendChild(modal);
        ...
        // ── Profile Overlay: Friend Code Section ────────────────────────
        // Format 6-char code as XXX-XXX for display
