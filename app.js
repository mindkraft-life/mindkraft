        ...
        //      ONLY used as anchor if it hasn't drifted past lastCompleted.
        //      Because processStreakShields stamps lastShieldCheckDate = today on
        //      every login, for inactive activities it continuously advances past
        //      lastCompleted. Using it as an anchor then makes _missedWindowsSince
        //      always see 0 closed windows, freezing shields permanently.
        //      Fix: if lastShieldCheckDate > lastCompleted, it has drifted and is
        //      no longer a valid shield checkpoint — fall through to lastCompleted.
        //   3. lastCompleted window — true last meaningful checkpoint.
        function _getStreakShieldWindow(activity) {
            ...
            if (activity.lastShieldCheckDate) {
                const checkDate = new Date(activity.lastShieldCheckDate + 'T00:00:00');
                // Drift guard: if the idempotency stamp has advanced past the last
                // completion, it is no longer a reliable shield anchor. Use
                // lastCompleted window instead to correctly count historical misses.
                if (activity.lastCompleted && checkDate > new Date(activity.lastCompleted)) {
                    const w = getCycleWindowStart(activity, new Date(activity.lastCompleted));
                    return w ? toLocalDateStr(w) : null;
                }
                const w = getCycleWindowStart(activity, checkDate);
                return w ? toLocalDateStr(w) : null;
            }
